...
const addTwo = (arr) => {
  ...
  for (let i = 0; i < arr.length; i++) {
    let newValue = arr[i] + 2; // Solution code here...
...
function containsAnd(arr) {
  let newFilter = arr.filter(str => str.includes('and'));
  return newFilter;// Solution code here...
}
...
const oddValues = (arr) => {
  let newFilter = arr.filter(num => num % 2 === 1);
  return newFilter;// Solution code here...
};
