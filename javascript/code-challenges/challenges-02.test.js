...
const raisedToTheThird = (arr) => {
  ...
  arr.forEach(newPower => arrayOfNumbers.push(Math.pow(newPower, 3)));
...
const addOne = (arr) => {
  let arrayOfNumbers = arr.map(addedNumber => addedNumber + 1);// Solution code here...
  return arrayOfNumbers;
};
...
const addQuestion = (arr) => {
  ...
  return arr.map(newString => newString + '?');
...
const forEachTwoToThe = (arr) => {
  let newestArray = [];
  arr.forEach(i => { newestArray.push(Math.pow(2, i));
  });// Solution code here...
...
const mapTwoToThe = (arr) => {
  return arr.map(newArr => Math.pow(2, newArr));// Solution code here...
};
